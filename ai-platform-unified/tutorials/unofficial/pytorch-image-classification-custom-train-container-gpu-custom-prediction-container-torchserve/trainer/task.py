...
def parse_args():
  parser = argparse.ArgumentParser(
        description='PyTorch Image Classification using GPU with Custom Container')
  parser.add_argument(
...
def train(model, criterion, optimizer, scheduler, dataset_sizes, dataloaders, device, epochs):
  since = time.time()
...
def main():
  ...
  image_datasets = load_dataset(dataset_dir)
  ...
  optimizer_ft = SGD(model_ft.parameters(), lr=lr, momentum=momentum)
  ...
  if args.model_dir:
    ...
    utils.upload_blob(
        ...
        destination_blob_name=os.path.join(gcs_blob_prefix, f'{model_name}')
    )
