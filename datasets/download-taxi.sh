...
if [[ $2 == 'big' ]]; then
  ...
  export BIG_TAXI_TRAINING=$1/big_taxi_trips_train.csv
  export BIG_TAXI_EVALUATION=$1/big_taxi_trips_eval.csv
elif [[ $2 == 'small' ]]; then
  ...
  export SMALL_TAXI_TRAINING=$1/small_taxi_trips_train.csv
  export SMALL_TAXI_EVALUATION=$1/small_taxi_trips_eval.csv
else
  echo "Downloading the big dataset..."
  gsutil cp ${DATA_FOLDER}/chicago_taxi/big_taxi_trips_train.csv big_taxi_trips_train.csv
  gsutil cp ${DATA_FOLDER}/chicago_taxi/big_taxi_trips_eval.csv big_taxi_trips_eval.csv
